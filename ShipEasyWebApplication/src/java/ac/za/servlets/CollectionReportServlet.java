    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       HttpSession session = request.getSession(false);
         ...
         if (session != null) {
             ...
             Shipment shipment = shipFacadeSBL.find(Long.parseLong(request.getParameter("collection_report")));
             
             if (shipment != null) {
                 
                 session.setAttribute("shipment", shipment);
                 
                 RequestDispatcher disp = request.getRequestDispatcher("shipment_collection_form.jsp");
